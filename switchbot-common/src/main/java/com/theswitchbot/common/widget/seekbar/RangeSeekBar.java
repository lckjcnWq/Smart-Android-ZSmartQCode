package com.theswitchbot.common.widget.seekbar;
...
import static com.theswitchbot.common.widget.seekbar.SeekBar.INDICATOR_MODE_ALWAYS_SHOW;
...
//    private float maxPositiveValue, minPositiveValue;
    ...
    private int leftThumbDrawable, rightThumbDrawable, leftThumbDisableDrawable, rightThumbDisableDrawable;
    ...
    public RangeSeekBar(Context context, AttributeSet attrs) {
        ...
        if (seekBarMode == SEEKBAR_MODE_RANGE) {
            ...
            leftSB.setThumbMode(2);
            rightSB.setThumbMode(1);
            if(leftThumbDrawable != -1){
                leftSB.setThumbDrawableId(leftThumbDrawable);
            }
            if(rightThumbDrawable != -1){
                rightSB.setThumbDrawableId(rightThumbDrawable);
            }
        } else {
            leftSB = new SeekBar(this, attrs, true);
            rightSB = null;
    ...
    //倒转颜色
    public void setInverseState(boolean reverse) {
        //进度中进度条的颜色
        //the color of seekBar in progress
//        private int progressColor;
        ...
        //the default color of the progress bar
//        private int progressDefaultColor;
        if (reverse != mReverseStatus) {
            int temp = progressColor;
            progressColor = progressDefaultColor;
            progressDefaultColor = temp;
            invalidate();
            mReverseStatus = !mReverseStatus;
        }
    }
    ...
    private void initProgressLine() {
        ...
        if (rightSB == null) {
            lineTop = (int) (leftSB.getIndicatorHeight() + leftSB.getIndicatorArrowSize() + leftSB.getThumbHeight() * leftSB.getThumbScaleRatio() / 2 - progressHeight / 2);
        } else {
            lineTop = (int) (Math.max(leftSB.getIndicatorHeight() + leftSB.getIndicatorArrowSize() + leftSB.getThumbHeight() * leftSB.getThumbScaleRatio() / 2, rightSB.getIndicatorHeight() + rightSB.getIndicatorArrowSize() + rightSB.getThumbHeight() / 2) - progressHeight / 2);
        }
    ...
    private void initAttrs(AttributeSet attrs) {
        ...
        disableColor = t.getColor(R.styleable.RangeSeekBar_rsb_disable_color, 0xFFC8C8C8);
        ...
        leftThumbDrawable = t.getResourceId(R.styleable.RangeSeekBar_leftThumbDrawable, -1);
        rightThumbDrawable = t.getResourceId(R.styleable.RangeSeekBar_rightThumbDrawable, -1);
        leftThumbDisableDrawable = t.getResourceId(R.styleable.RangeSeekBar_leftThumbDisableDrawable, leftThumbDrawable);
        rightThumbDisableDrawable = t.getResourceId(R.styleable.RangeSeekBar_rightThumbDisableDrawable, rightThumbDrawable);
        t.recycle();
    }
...
//            rightSB.setShowIndicatorEnable(false);
            rightSB.setIndicatorText(null);
        }
        leftSB.setIndicatorText(null);
//        leftSB.setShowIndicatorEnable(false);
    ...
    public void setEnabled(boolean enabled) {
        ...
        leftSB.setEnabled(enabled);
        leftSB.setThumbDrawableId(enabled ? leftThumbDrawable : leftThumbDisableDrawable);
        rightSB.setEnabled(enabled);
        rightSB.setThumbDrawableId(enabled ? rightThumbDrawable : rightThumbDisableDrawable);
        invalidate();
    }

    public void setLeftIndicatorText(String progress) {
        if (leftSB != null) {
            leftSB.setIndicatorText(progress);
        }
    }

    public void setRightIndicatorText(String progress) {
        if (rightSB != null) {
                ...
                if (currTouchSB == leftSB) {
                    ...
                    if (tickMarkNumber > 1) {
                        ...
                        while (touchRightCellsValue < currLeftCellsValue + minRangeCells) {
                            ...
                            if (touchRightCellsValue > (maxProgress - minProgress)) {
                                break;
                            }
                            percent = touchRightCellsValue * cellsPercent;
                        }
