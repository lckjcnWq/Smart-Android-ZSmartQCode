package com.theswitchbot.common.widget.countdownview;
 ...
 * 2020年4月27日11:26:58
 * 修改内容如下：
 * 1. 增加formatString，根据theFormatElement来将formatString分成前后两个部分，增加到整个控件的头尾
 * 2. 强制让suffix和前后数字、文案对齐，取消他们的gravity
 */
class BaseCountdown {
    ...
    protected float mSuffixMillisecondLeftMargin, mSuffixMillisecondRightMargin;
    ...
    protected float mFormatStringHeadTextWidth, mFormatStringTailTextWidth;
    protected float mFormatStringPreRightMargin, mFormatStringSubLeftMargin;
    private static final String THE_DEFAULT_FORMAT_ELEMENT = "%t";
    private String theFormatElement = THE_DEFAULT_FORMAT_ELEMENT;
    ...
    public void initStyleAttr(Context context, TypedArray ta) {
        ...
        mSuffixMillisecondRightMargin = ta.getDimension(R.styleable.CountdownView_suffixMillisecondRightMargin, -1);
        formatString = ta.getString(R.styleable.CountdownView_formatString);
        mFormatStringPreRightMargin = ta.getDimension(R.styleable.CountdownView_formatStringPreRightMargin, -1);
        mFormatStringSubLeftMargin = ta.getDimension(R.styleable.CountdownView_formatStringSubLeftMargin, -1);
        theFormatElement = ta.getString(R.styleable.CountdownView_theFormatElement);
        if(theFormatElement == null) theFormatElement = THE_DEFAULT_FORMAT_ELEMENT;
...
//        if (!isShowMinute && !isShowSecond) isShowSecond = true;
    ...
    private void initSuffixMargin() {
        ...
        if(formatString != null){
            String[] ff = formatString.split(theFormatElement, 2);
            String pre = ff[0];
            if(pre != null) {
                if (mFormatStringPreRightMargin < 0) {
                    if (!isSuffixLRMarginNull) {
                        mFormatStringPreRightMargin = mSuffixLRMargin;
                    } else {
                        mFormatStringPreRightMargin = defSuffixLRMargin;
                    }
                }
            } else{
                mFormatStringPreRightMargin = 0;
            }
//            if(ff.length > 1) {
//                if (mFormatStringSubLeftMargin < 0) {
//                    if (!isSuffixLRMarginNull) {
//                        mFormatStringSubLeftMargin = mSuffixLRMargin;
//                    } else {
//                        mFormatStringSubLeftMargin = defSuffixLRMargin;
//                    }
//                }
//                if (mSuffixMillisecondRightMargin < 0) {
//                    if (!isSuffixLRMarginNull) {
//                        mSuffixMillisecondRightMargin = mSuffixLRMargin;
//                    } else {
//                        mSuffixMillisecondRightMargin = defSuffixLRMargin;
//                    }
//                }
//            } else{
                mFormatStringSubLeftMargin = 0;
                mSuffixMillisecondRightMargin = 0;
//            }
        } else{
            mFormatStringPreRightMargin = 0;
            mFormatStringSubLeftMargin = 0;
    ...
    protected void initTimeTextBaseInfo() {
        ...
        if(formatString != null){
            mTimeTextPaint.getTextBounds(formatString, 0, formatString.length(), rect2);
            mTimeTextHeight = Math.max(rect.height(), rect2.height());
            mTimeTextBottom = Math.max(rect.bottom, rect2.bottom);
        }
    }
    ...
    protected final float getAllContentWidthBase(float timeWidth) {
        ...
        width += (mSuffixDayLeftMargin + mSuffixDayRightMargin + mSuffixHourLeftMargin + mSuffixHourRightMargin
                + mSuffixMinuteLeftMargin + mSuffixMinuteRightMargin + mSuffixSecondLeftMargin + mSuffixSecondRightMargin
                + mSuffixMillisecondLeftMargin + mFormatStringPreRightMargin + mFormatStringSubLeftMargin
                + mSuffixMillisecondRightMargin);
        ...
        if(formatString != null){
            width += getFormatStringContentWidth();
        }
        return width;
    }
    ...
    private float getFormatStringContentWidth(){
        float width = 0;
        Rect tempRect = new Rect();
        String[] ff = formatString.split(theFormatElement, 2);
        String pre = ff[0];
        mTimeTextPaint.getTextBounds(pre, 0, pre.length(), tempRect);
        mFormatStringHeadTextWidth = tempRect.width();
        width += mFormatStringHeadTextWidth;
        if(ff.length > 1){
            String sub = ff[1];
            mTimeTextPaint.getTextBounds(sub, 0, sub.length(), tempRect);
            mFormatStringTailTextWidth = tempRect.width();
            width += mFormatStringTailTextWidth;
        }
        return width;
    }
    ...
    public void onDraw(Canvas canvas) {
        ...
        float mMinuteLeft;
        float mSecondLeft;
        float mFormatPreRight;
        float mSecondRight;

        String[] ss;
        if(formatString != null){
            ss = formatString.split(theFormatElement, 2);
            String pre = ss[0];
            canvas.drawText(pre, mLeftPaddingSize + mFormatStringHeadTextWidth / 2, mTimeTextBaseline, mTimeTextPaint);
            mFormatPreRight = mLeftPaddingSize + mFormatStringHeadTextWidth + mFormatStringPreRightMargin;
        } else{
            mFormatPreRight = mLeftPaddingSize;
        }

        if (isShowDay) {
            // draw day text
            canvas.drawText(Utils.formatNum(mDay), mFormatPreRight + mDayTimeTextWidth / 2, mTimeTextBaseline, mTimeTextPaint);
            if (mSuffixDayTextWidth > 0) {
                // draw day suffix
                canvas.drawText(mSuffixDay, mFormatPreRight + mDayTimeTextWidth + mSuffixDayLeftMargin, mTimeTextBaseline, mSuffixTextPaint);
            }

            // hour left point
//            mHourLeft = mLeftPaddingSize + mDayTimeTextWidth + mSuffixDayTextWidth + mSuffixDayLeftMargin + mSuffixDayRightMargin;
            mHourLeft = mFormatPreRight + mDayTimeTextWidth + mSuffixDayTextWidth + mSuffixDayLeftMargin + mSuffixDayRightMargin;
        } else {
            // hour left point
//            mHourLeft = mLeftPaddingSize;
            mHourLeft = mFormatPreRight;
        }
        ...
        if (isShowHour) {
            ...
            if (mSuffixHourTextWidth > 0) {
                // draw hour suffix
                canvas.drawText(mSuffixHour, mHourLeft + curTimeTextWidth + mSuffixHourLeftMargin, mTimeTextBaseline, mSuffixTextPaint);
            }
        ...
        if (isShowMinute) {
            ...
            if (mSuffixMinuteTextWidth > 0) {
                // draw minute suffix
                canvas.drawText(mSuffixMinute, mMinuteLeft + mTimeTextWidth + mSuffixMinuteLeftMargin, mTimeTextBaseline, mSuffixTextPaint);
            }
        ...
        if (isShowSecond) {
            ...
            if (mSuffixSecondTextWidth > 0) {
                // draw second suffix
                canvas.drawText(mSuffixSecond, mSecondLeft + mTimeTextWidth + mSuffixSecondLeftMargin, mTimeTextBaseline, mSuffixTextPaint);
            }
            ...
            if (isShowMillisecond) {
                ...
                if (mSuffixMillisecondTextWidth > 0) {
                    ...
                    canvas.drawText(mSuffixMillisecond, mMillisecondLeft + mTimeTextWidth + mSuffixMillisecondLeftMargin, mTimeTextBaseline, mSuffixTextPaint);
                }
                mSecondRight = mMillisecondLeft + mTimeTextWidth + mSuffixMillisecondLeftMargin + mSuffixMillisecondRightMargin;
            } else{
                mSecondRight = mSecondLeft + mTimeTextWidth + mSuffixSecondTextWidth + mSuffixSecondLeftMargin;
            }
        } else{
            mSecondRight = mSecondLeft;
        }
        ...
        if(formatString != null){
            ...
            if(ss.length > 1){
                String sub = ss[1];
                canvas.drawText(sub, mSecondRight + mFormatStringTailTextWidth / 2 + mFormatStringSubLeftMargin, mTimeTextBaseline, mTimeTextPaint);
            }
        }
    }
